...
from youtube_ingest import resolve_video_id  # noqa: E402
...
def _video_dir(character: str, video_id: str) -> Path:
    _check_name(character, "character")
    _check_name(video_id, "video_id")
    video_dir = WORK_DIR / character / "youtube" / video_id
    if not video_dir.exists():
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"No ingested video {video_id} for {character}"
        )
    return video_dir


def _review_path(character: str, video_id: str) -> Path:
    review_path = _video_dir(character, video_id) / "review.csv"
...
@app.get("/characters/{character}/videos/{video_id}/clips")
async def get_clips(character: str, video_id: str) -> dict:
    rows = read_review_csv(_review_path(character, video_id))
    map_path = _video_dir(character, video_id) / SPEAKER_MAP_FILENAME
    speaker_map = (
...
@app.patch("/characters/{character}/videos/{video_id}/clips")
async def patch_clips(
    character: str, video_id: str, decisions_request: ClipDecisionRequest
) -> dict:
    review_path = _review_path(character, video_id)
...
@app.put("/characters/{character}/videos/{video_id}/speaker-map")
async def put_speaker_map(
    character: str, video_id: str, map_request: SpeakerMapRequest
) -> dict:
    for target in map_request.speaker_map.values():
        if target is not None:
            _check_name(target, "character")
    written = write_speaker_map(
        _video_dir(character, video_id), map_request.speaker_map
    )
    return {"speaker_map": map_request.speaker_map, "path": str(written)}


@app.get("/characters/{character}/videos/{video_id}/clips/{clip_id}/audio")
async def get_clip_audio(character: str, video_id: str, clip_id: str) -> FileResponse:
    _check_name(clip_id, "clip_id")
    clip_path = _video_dir(character, video_id) / "clips" / f"{clip_id}.wav"
