...
main.py imports chatterbox-tts, whisper, and piper_phonemize at module load
time (unlike api.py, see test_api.py's docstring) -- but those are ordinary
dependencies of this project's own uv environment, so importing main.py
...
# main.py lives one directory up from tests/, and is not itself a package --
...
def test_metadata_fingerprint_reports_clip_count_and_a_content_hash(tmp_path):
    ...
    fingerprint = main._metadata_fingerprint(tmp_path)
...
def test_metadata_fingerprint_changes_when_the_content_changes(tmp_path):
    ...
    before = main._metadata_fingerprint(tmp_path)

    write_metadata(tmp_path, ["id1|line one", "id2|line two"])
    after = main._metadata_fingerprint(tmp_path)
...
def test_metadata_fingerprint_raises_a_readable_error_when_metadata_csv_is_missing(
    ...
    with pytest.raises(SystemExit, match="resample"):
        main._metadata_fingerprint(tmp_path)
...
def test_load_sidecar_returns_none_when_no_sidecar_exists(tmp_path):
    assert main._load_sidecar(tmp_path) is None


def test_load_sidecar_treats_corrupt_json_as_absent(tmp_path):
    (tmp_path / main.DATASET_FINGERPRINT_NAME).write_text(
...
def test_load_sidecar_treats_invalid_utf8_bytes_as_absent(tmp_path):
    ...
    (tmp_path / main.DATASET_FINGERPRINT_NAME).write_bytes(b"\xff\xfe\x00\x01")
...
def test_write_sidecar_then_load_sidecar_round_trips(tmp_path):
    ...
    main._write_sidecar(tmp_path, fingerprint)

    assert main._load_sidecar(tmp_path) == fingerprint
...
def test_write_sidecar_leaves_no_temp_file_behind(tmp_path):
    ...
    main._write_sidecar(tmp_path, {"clip_count": 1, "metadata_hash": "x"})

    assert (tmp_path / main.DATASET_FINGERPRINT_NAME).exists()
    assert not (tmp_path / (main.DATASET_FINGERPRINT_NAME + ".tmp")).exists()
...
def test_stage_preprocess_runs_docker_and_writes_a_sidecar_on_first_run(
    ...
    monkeypatch.setattr(main, "run_docker", fake_run_docker)

    main.stage_preprocess("janeway")

    assert calls, "run_docker should have been called"
    assert main._load_sidecar(training_dir) == main._metadata_fingerprint(resampled_dir)
...
def test_stage_preprocess_skips_when_the_fingerprint_already_matches(
    ...
    training_dir = tmp_path / "work" / "janeway" / "training"
    write_config(training_dir)
    main._write_sidecar(training_dir, main._metadata_fingerprint(resampled_dir))
    ...
    monkeypatch.setattr(main, "run_docker", fail_run_docker)

    main.stage_preprocess("janeway")  # must not raise
...
def test_stage_preprocess_does_not_write_a_sidecar_when_run_docker_fails(
    ...
    monkeypatch.setattr(main, "run_docker", broken_run_docker)

    with pytest.raises(RuntimeError):
        main.stage_preprocess("janeway")

    assert main._load_sidecar(training_dir) is None
